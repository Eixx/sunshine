...
import com.example.cpe.sunshine.app.data.WeatherDbHelper;
...
public class TestDb extends AndroidTestCase {
  ...
  public void testInsertReadDb() {
    ...
    values.put(LocationEntry.COLUMN_LOCATION_SETTINGS, testLocationSetting);
    ...
    if (cursor.moveToFirst()) {
      ...
      int locationIndex = cursor.getColumnIndex(LocationEntry.COLUMN_LOCATION_SETTINGS);
    ...
    long weatherRow = db.insert(WeatherEntry.TABLE_NAME, null, weatherValues);
    ...
    if (weatherCursor.moveToFirst()) {
      int dateTextId = weatherCursor.getColumnIndex(WeatherEntry.COLUMN_DATETEXT);
      String dateText = weatherCursor.getString(dateTextId);

      int shortDescId= weatherCursor.getColumnIndex(WeatherEntry.COLUMN_SHORT_DESC);
      String shortDesc = weatherCursor.getString(shortDescId);

      int weatherId= weatherCursor.getColumnIndex(WeatherEntry.COLUMN_WEATHER_ID);
      int weather = weatherCursor.getInt(weatherId);

      assertEquals("Asteroids", shortDesc);
      assertEquals("20141205", dateText);
      assertEquals(321, weather);
    } else {
      fail("No result :(");
    }
